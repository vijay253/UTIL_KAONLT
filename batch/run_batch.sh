...
##Output batch script##
batch="sjdkay_Job.txt"

##Input run numbers##
inputFile="/w/hallc-scifs17exp/kaon/sjdkay/hallc_replay_kaonlt/UTIL_KAONLT/batch/inputRuns"
...
while true; do
    ...
    case $yn in
		...
		echo ""
		#eval "jcache pendingRequest sjdkay 2>/dev/null"
		##Run number#
		runNum=$line
		tmp=tmp 
		...
		echo "CPU: 2" >> ${batch}
		#echo "TIME: 1" >> ${batch}
		echo "COMMAND:/w/hallc-scifs17exp/kaon/sjdkay/hallc_replay_kaonlt/UTIL_KAONLT/KaonYield_batch.sh ${runNum}" >> ${batch}
		echo "MAIL: sjdkay@jlab.org" >> ${batch}    
		...
		rnum=($string)
		#echo "array is ${rnum[@]}"   
		#eval "jobstat -u sjdkay 2>/dev/null"
		eval "jobstat -u sjdkay 2>/dev/null" > ${tmp} 
		...
		if [ $i == $numlines ]; then
			...
			check="/home/sjdkay/.farm_out/KaonLT_${rnum[$l]}.$j.err"	   
			...
			if [ -e "$check" ]; then
			    ...
			    echo "Errors..." >> ${output}
			    echo " " >> ${output}
			    cat /home/sjdkay/.farm_out/KaonLT_${rnum[$l]}.$j.err >> ${output}
			    echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" >> ${output}
