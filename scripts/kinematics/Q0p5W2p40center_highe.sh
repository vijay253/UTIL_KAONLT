...
if [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/Q0p5W2p40center_highe_Kaons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q0p5W2p40center_highe_Kaons\")"
elif [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/Q0p5W2p40center_highe_Kaons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q0p5W2p40center_highe_Kaons\")"
else echo "Kaon plots already found in - ${UTILPATH}/OUTPUT/Analysis/KaonLT/Q0p5W2p40center_highe_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
