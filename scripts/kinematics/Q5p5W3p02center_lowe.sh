...
if [ ! -f "${UTILPATH}/scripts/kaonyield/OUTPUT/Q5p5W3p02center_lowe_Kaons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q5p5W3p02center_lowe_Kaons\")"
elif [ ! -f "${UTILPATH}/scripts/kaonyield/OUTPUT/Q5p5W3p02center_lowe_Kaons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q5p5W3p02center_lowe_Kaons\")"
else echo "Kaon plots already found in - ${UTILPATH}/scripts/kaonyield/OUTPUT/Q5p5W3p02center_lowe_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
