...
RunListFile="${UTILPATH}/scripts/kinematics/Q2p115W2p95center_highe"
...
if [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/Q2p115W2p95center_highe_Kaons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q2p115W2p95center_highe_Kaons\")"
elif [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/Q2p115W2p95center_highe_Kaons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q2p115W2p95center_highe_Kaons\")"
else echo "Kaon plots already found in - ${UTILPATH}/OUTPUT/Analysis/KaonLT/Q2p115W2p95center_highe_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
