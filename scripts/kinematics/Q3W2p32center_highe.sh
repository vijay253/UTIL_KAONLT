...
if [ ! -f "${UTILPATH}/scripts/kaonyield/OUTPUT/Q3W2p32center_highe_Kaons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q3W2p32center_highe_Kaons\")"
elif [ ! -f "${UTILPATH}/scripts/kaonyield/OUTPUT/Q3W2p32center_highe_Kaons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q3W2p32center_highe_Kaons\")"
else echo "Kaon plots already found in - ${UTILPATH}/scripts/kaonyield/OUTPUT/Q3W2p32center_highe_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
