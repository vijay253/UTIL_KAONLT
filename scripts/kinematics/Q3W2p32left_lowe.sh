...
if [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/Q3W2p32left_lowe_Kaons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q3W2p32left_lowe_Kaons\")"
elif [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/Q3W2p32left_lowe_Kaons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q3W2p32left_lowe_Kaons\")"
else echo "Kaon plots already found in - ${UTILPATH}/OUTPUT/Analysis/KaonLT/Q3W2p32left_lowe_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
