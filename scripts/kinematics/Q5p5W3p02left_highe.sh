...
if [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/Q5p5W3p02left_highe_Kaons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q5p5W3p02left_highe_Kaons\")"
elif [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/Q5p5W3p02left_highe_Kaons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q5p5W3p02left_highe_Kaons\")"
else echo "Kaon plots already found in - ${UTILPATH}/OUTPUT/Analysis/KaonLT/Q5p5W3p02left_highe_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
