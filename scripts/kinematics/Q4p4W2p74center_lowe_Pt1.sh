...
if [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/Q4p4W2p74center_lowe_Pt1_Kaons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q4p4W2p74center_lowe_Pt1_Kaons\")"
elif [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/Q4p4W2p74center_lowe_Pt1_Kaons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q4p4W2p74center_lowe_Pt1_Kaons\")"
else echo "Kaon plots already found in - ${UTILPATH}/OUTPUT/Analysis/KaonLT/Q4p4W2p74center_lowe_Pt1_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
