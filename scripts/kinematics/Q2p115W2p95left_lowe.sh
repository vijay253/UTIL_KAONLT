...
if [ ! -f "${UTILPATH}/scripts/kaonyield/OUTPUT/Q2p115W2p95left_lowe_Kaons.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q2p115W2p95left_lowe_Kaons\")"
elif [ ! -f "${UTILPATH}/scripts/kaonyield/OUTPUT/Q2p115W2p95left_lowe_Kaons.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"Q2p115W2p95left_lowe_Kaons\")"
else echo "Kaon plots already found in - ${UTILPATH}/scripts/kaonyield/OUTPUT/Q2p115W2p95left_lowe_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
