...
if len(sys.argv)-1!=3:
    print("!!!!! ERROR !!!!!\n Expected 3 arguments\n Usage is with - RunNumber MaxEvents CutFileName\n!!!!! ERROR !!!!!")
    sys.exit(1)
...
OUTPATH = "%s/UTIL_PROTON/scripts/kaonyield/OUTPUT" % REPLAYPATH
sys.path.insert(0, '%s/UTIL_PROTON/bin/python/' % REPLAYPATH)
import kaonlt as klt

print("Running as %s on %s, hallc_replay_lt path assumed as %s" % (USER[1], HOST[1], REPLAYPATH))
rootName = "%s/UTIL_KAONLT/ROOTfilesProton/Proton_coin_replay_production_%s_%s.root" % (REPLAYPATH, runNum, MaxEvent)
ConfigFile = "%s/UTIL_KAONLT/config/%s" % (REPLAYPATH,  ConfigFileName)
...
for line in Configf:
    ...
    if ("Timing_Cuts" in array[0]):
        TimingTmp = array[1]
        TimingCutFile = "%s/UTIL_KAONLT/config/%s" % (REPLAYPATH,TimingTmp)
    if ("PID_Cuts" in array[0]):
        PIDTmp = array[1]
        PIDCutFile = "%s/UTIL_KAONLT/config/%s" % (REPLAYPATH,PIDTmp)
    if ("Acceptance_Cuts" in array[0]):
        AcceptTmp = array[1]
        AcceptCutFile = "%s/UTIL_KAONLT/config/%s" % (REPLAYPATH,AcceptTmp)
Configf.close()
try:
    TimingCutFile
    PIDCutFile
    ...
    print("!!!!! ERRROR !!!!!\n One (or more) of the cut files is not defined in the config file provided\n!!!!! ERRORR !!!!!")
    sys.exit(2)
