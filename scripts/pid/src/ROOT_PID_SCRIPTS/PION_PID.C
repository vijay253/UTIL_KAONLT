...
void PION_PID(string InFilename = "", string OutFilename = "")
{
  ...
  if(Hostname.Contains("farm")){
    ...
    Outpath1 = Replaypath+"/UTIL_KAONLT/scripts/pid/src/ROOT_PID_SCRIPTS/OUTPUT";
  }
  ...
  */
  ...
  std::ofstream HGC_PI_EFF;
  HGC_PI_EFF.open("HGC_PION_EFF.csv", ios::out);
  
  if (!HGC_PI_EFF.is_open()) cout << "Problem in opening file!!!" << endl;

  else
    {
      //   HGC_PI_EFF << "Efficiencyineachbin =  " <<endl; 
      //      HGC_PI_EFF << ("X_BIN") <<endl; 
      
      for (Double_t i = 1; i < 81; i++)  // x bins
	{
	  HGC_PI_EFF << Form("X_BIN_%1.0f ",i); 
	  // HGC_PI_EFF << ("YATCERALLBIN") <<endl; 
     
	  for (Double_t j = 1; j < 61; j++)  // x bins
	    {
	      Double_t Eff = H2_HGC_EFF1->GetBinContent(i, j);
	      Double_t Eff_ERR = H2_HGC_EFF1->GetBinError(i, j);
	      //cout << " HGC PION EFF =  "<< Eff << " +/- " << Eff_ERR << endl;
	      
	      //	      HGC_PI_EFF << Form("%1.2f;%1.1f,",Eff, Eff_ERR); if (j == 60) {HGC_PI_EFF <<endl;}
	      HGC_PI_EFF << Form("%1.2f,",Eff); if (j == 60) {HGC_PI_EFF <<endl;}
  ...
  cout<< "AEROGEL PION EFF = " << (PI_AERO_X1/PI_AERO_Y1)*100 << " +/- "<< sqrt(1/(PI_AERO_X1) + 1/(PI_AERO_Y1))*100<<endl;
