...
void SCRIPT_KAON_PID(string InFilename = "", string OutFilename = "")
{
  ...
  TH1D *H1_KAON_MM1                    = new TH1D("H1_KAON_MM1"," KAON MM ; P_kin_secondary_MMK;", 300, 0.0, 2.0);
  TH1D *H1_KAON_MM2                    = new TH1D("H1_KAON_MM2"," KAON MM (HOLE CUT OFF); P_kin_secondary_MMK;", 300, 0.0, 2.0);
  TH1D *H1_EKAON_COIN                  = new TH1D("H1_EKAON_COIN"," EKAON COIN TIME; P_CTime_eKaon;", 300, -40.0, 40.0);
  ...
  for(Long64_t i = 0; i < nEntries_SHMS_EVENTS; i++)
    {
     ...
     //CUTS      
      ...
      HGC_CUT2 = P_CTime_eKaon > -2.0 && P_CTime_eKaon < 2.0 && P_cal_etottracknorm <0.7; 
      ...
      HGC_CUT7 = RFTime_SHMS_RFtimeDist > 1.50 && RFTime_SHMS_RFtimeDist < 2.43;
     
      PI_DENO_HGC = HGC_CUT1 && HGC_CUT2 &&  HGC_CUT3 && HGC_CUT4 && HGC_CUT5 && HGC_CUT7; 
      PI_NUM_HGC  =  P_hgcer_npeSum < 0.1 && PI_DENO_HGC;
    
      if(P_hgcer_npeSum > pow(10, -6) && HGC_CUT1 && HGC_CUT2 && HGC_CUT3)
	{
      ...
      if(P_hgcer_npeSum < 0.1 && HGC_CUT1 && HGC_CUT2 && HGC_CUT4 && HGC_CUT5 && HGC_CUT7)
	{
	  ...
	  H2_MM_RFTime->Fill(P_kin_secondary_MMK, RFTime_SHMS_RFtimeDist);
	}
  ...
  cout<< "HGC KAON EFF          = " << (PI_HGC_X1/PI_HGC_Y1)*100 << " +/- "<< sqrt(1/(PI_HGC_X1) + 1/(PI_HGC_Y1))*100<<endl;

  Double_t PI_HGC_X2 =  H1_HG_NPE_NUM2->GetEntries(); 
  Double_t PI_HGC_Y2 =  H1_HG_NPE_DENO2->GetEntries(); 

  cout<< "HGC KAON EFF (MM CUT) = " << (PI_HGC_X2/PI_HGC_Y2)*100 << " +/- "<< sqrt(1/(PI_HGC_X2) + 1/(PI_HGC_Y2))*100<<endl;
  ...
  */
  ...
  TAxis *MMAxis1 = H1_KAON_MM2->GetXaxis();
  Int_t Lowx1   = MMAxis1->FindBin(0.901);    // select pion peak
  Int_t Highx1  = MMAxis1->FindBin(1.054);
  Double_t Pi_mm_BinIntegral1 = H1_KAON_MM2->Integral(Lowx1, Highx1);

  TAxis *MMAxis2 = H1_KAON_MM1->GetXaxis();
  Int_t Lowx2   = MMAxis2->FindBin(0.901);    // select pion peak
  Int_t Highx2  = MMAxis2->FindBin(1.054);
  Double_t Pi_mm_BinIntegral2 = H1_KAON_MM1->Integral(Lowx2, Highx2);
  ...
  for(Long64_t i = 0; i < nEntries_SHMS_EVENTS; i++)
    {
      ...
      AERO_CUT2 = P_CTime_eKaon > -2.0 && P_CTime_eKaon < 2.0 && P_cal_etottracknorm <0.7; 
      AERO_CUT3 = RFTime_SHMS_RFtimeDist > 1.50 && RFTime_SHMS_RFtimeDist < 2.43;
  ...
  cout<< "AEROGEL KAON EFF = " << (PI_AERO_X1/PI_AERO_Y1)*100 << " +/- "<< sqrt(1/(PI_AERO_X1) + 1/(PI_AERO_Y1))*100<<endl;
