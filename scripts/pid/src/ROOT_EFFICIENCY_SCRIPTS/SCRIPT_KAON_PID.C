// .... Created Date: May 18, 2021 ....
// .... Author: Vijay Kumar ....
//''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
//''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
#define SCRIPT_KAON_PID_cxx
#include <TStyle.h>
#include <TCanvas.h>
#include <TLine.h>
#include <TMath.h>
...
//#################################################################### 
  ...
  */
  ...
  TH1D *H1_KAON_MM1                    = new TH1D("H1_KAON_MM1"," KAON MM ; KAON MM;", 300, 0.0, 2.0);
  TH2D *H2_HG_AERO_CHER                = new TH2D("H2_HG_AERO_CHER","HG VS AERO CHER; HG NPE; AERO NPE;", 80, 0, 40.0, 80, 0, 40.0); 
  ...
  for(Long64_t i = 0; i < nEntries_SHMS_EVENTS; i++)
    {
      ...
      HGC_CUT3 = K_mm > 1.05 && K_mm < 1.25;
      HGC_CUT4 = P_aero_npeSum  < 4.0;
	...
	{
	  H2_HG_AERO_CHER->Fill(P_hgcer_npeSum, P_aero_npeSum);
	}
  ...
  */
  ...
  TAxis *MMAxis = H1_KAON_MM1->GetXaxis();
  Int_t Lowx   = MMAxis->FindBin(0.901);    // select pion peak
  Int_t Highx  = MMAxis->FindBin(1.054);
  Double_t Pi_mm_BinIntegral = H1_KAON_MM1->Integral(Lowx, Highx);
  ...
  for(Long64_t i = 0; i < nEntries_SHMS_EVENTS; i++)
    {
      ...
      PI_NUM_AERO  =  P_aero_npeSum < 4.0 && PI_DENO_AERO;
