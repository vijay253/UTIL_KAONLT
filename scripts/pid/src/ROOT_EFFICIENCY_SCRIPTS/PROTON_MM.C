...
void PROTON_MM(string InFilename = "", string OutFilename = "")
{
  ...
  // Particles information with "PROTON" pid 
  ...
  Double_t P_hgcer_xAtCer;Proton_No_HGC_Cut->SetBranchAddress("P_hgcer_xAtCer", &P_hgcer_xAtCer);
  Double_t P_hgcer_yAtCer;Proton_No_HGC_Cut->SetBranchAddress("P_hgcer_yAtCer", &P_hgcer_yAtCer);
  Double_t P_aero_yAtCer;Proton_No_HGC_Cut->SetBranchAddress("P_aero_yAtCer", &P_aero_yAtCer);
  Double_t Proton_npeSum;Proton_No_HGC_Cut->SetBranchAddress("P_hgcer_npeSum", &Proton_npeSum);
  Double_t Proton_aero_npeSum;Proton_No_HGC_Cut->SetBranchAddress("P_aero_npeSum", &Proton_aero_npeSum);
  Double_t Proton_gtr_p; Proton_No_HGC_Cut->SetBranchAddress("P_gtr_p", &Proton_gtr_p);
  Double_t Proton_e_miss;Proton_No_HGC_Cut->SetBranchAddress("emiss", &Proton_e_miss);
  Double_t Proton_p_miss;Proton_No_HGC_Cut->SetBranchAddress("pmiss", &Proton_p_miss);
  ...
  for(Long64_t i = 0; i < nEntries_Proton_No_HGC_Cut; i++){
    ...
    if (Proton_npeSum > 1.5 || Proton_aero_npeSum > 1.0 || P_aero_yAtCer >31) continue;
    h1_MM_P_IN_TCutG1->Fill(sqrt(pow((Proton_e_miss + (sqrt((MK*MK) + (Proton_gtr_p*Proton_gtr_p))) - (sqrt((Mp*Mp) + (Proton_gtr_p*Proton_gtr_p)))), 2) - (Proton_p_miss*Proton_p_miss)));
  }
  for(Long64_t i = 0; i < nEntries_Proton_No_HGC_Cut; i++){
    Proton_No_HGC_Cut->GetEntry(i);
    if (cutg2->IsInside(P_hgcer_yAtCer, P_hgcer_xAtCer)) continue;
    if (Proton_npeSum > 1.5 || Proton_aero_npeSum > 2.0 || P_aero_yAtCer >31) continue;
    h1_MM_P_OUT_TCutG2->Fill(sqrt(pow((Proton_e_miss + (sqrt((MK*MK) + (Proton_gtr_p*Proton_gtr_p))) - (sqrt((Mp*Mp) + (Proton_gtr_p*Proton_gtr_p)))), 2) - (Proton_p_miss*Proton_p_miss)));
}
    for(Long64_t i = 0; i < nEntries_Proton_No_HGC_Cut; i++){
    Proton_No_HGC_Cut->GetEntry(i);
    if(!cutg2->IsInside(P_hgcer_yAtCer, P_hgcer_xAtCer)) continue;   
    if (cutg1->IsInside(P_hgcer_yAtCer, P_hgcer_xAtCer)) continue;  
    if (Proton_npeSum > 1.5 || Proton_aero_npeSum > 2.0 ||P_aero_yAtCer >31) continue;
    h1_MM_P_IN_TCutG12->Fill(sqrt(pow((Proton_e_miss + (sqrt((MK*MK) + (Proton_gtr_p*Proton_gtr_p))) - (sqrt((Mp*Mp) + (Proton_gtr_p*Proton_gtr_p)))), 2) - (Proton_p_miss*Proton_p_miss)));
  }

  //Write the info in the root format
  TFile *OutHisto_file = new TFile(foutname,"RECREATE");
  TDirectory *PROTON_MM = OutHisto_file->mkdir("PROTON_MM");
  PROTON_MM->cd();
