void replay_elastics_coin (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/coin_replay_elastics_%d_%d.root";
 ...
 // Include golden track information
  ...
  // Add Physics Module to calculate secondary (scattered) beam kinematics
   THcPrimaryKine* kin_primary = new THcPrimaryKine("H.kin.primary", "HMS Single Arm Kinematics", "H", "H.rb");
   gHaPhysics->Add(kin_primary);
   THcSecondaryKine* kin_secondary = new THcSecondaryKine("P.kin.secondary", "SHMS Single Arm Kinematics", "P", "H.kin.primary");
   gHaPhysics->Add(kin_secondary);
  ...
  // Define DEF-file+
  analyzer->SetOdefFile("DEF-files/COIN/PRODUCTION/coin_elastics.def");
  // Define cuts file
  ...
  analyzer->PrintReport("TEMPLATES/COIN/PRODUCTION/coin_production.template",
  			Form("REPORT_OUTPUT/COIN/PRODUCTION/replay_coin_elastics_%d_%d.report", RunNumber, MaxEvent));  // optional
