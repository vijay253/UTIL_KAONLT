void replay_production_coin_TT (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/coin_replay_production_TT_%d_%d.root";
  ...
  // Define DEF-file+
  analyzer->SetOdefFile("UTIL_KAONLT/scripts_TrackingTest/coin_production_TT.def");
  // Define cuts file
  analyzer->SetCutFile("UTIL_KAONLT/scripts_TrackingTest/coin_production_TT_cuts.def");  // optional
  // File to record accounting information for cuts
  analyzer->SetSummaryFile(Form("REPORT_OUTPUT/COIN/PRODUCTION/summary_TT_%d_%d.report", RunNumber, MaxEvent));  // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template
  analyzer->PrintReport("TEMPLATES/COIN/PRODUCTION/coin_production.template",
  			Form("REPORT_OUTPUT/COIN/PRODUCTION/replay_coin_TT_%d_%d.report", RunNumber, MaxEvent));  // optional
